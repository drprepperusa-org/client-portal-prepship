...
|---|---|---|---|---|---|---|
| **Orders list/detail** | `orders` (`order_status`, `canonical_status`, `externally_shipped`, `raw.externallyFulfilled`) + normalized `order_items` + latest active OUTBOUND (`is_return = false`) `shipments` tracking; `order_overrides` tracking is legacy fallback only; `clients`/`stores` for names | `GET /orders` (`?status` whitelisted to `awaiting_shipment \| shipped \| cancelled`, else 400), `GET /orders/:id` → shared `toPortalOrderDto()` (`src/lib/client-portal/dto.ts`); status = `resolveOrderFulfillmentStatus()` over PrepShip's effective lifecycle (`src/lib/client-portal/order-lifecycle.ts`, pinned port of `prepship-v4` @ `dfc6809e`); tabs / `awaitingActiveOrderCount` on the same expression | `PortalOrder` (`items[]`, `orderedUnits`, `displayTrackingNumber`, `fulfillmentStatus` = `pending \| shipped \| cancelled \| voided`; `orderStatus` kept for the cost summary only, never rendered as a status) | `pages/Orders.tsx` (`portalApi.orders`/`order`) and shared `OrderDetailLoader` / `OrderDetailPanel`, both via `fulfillmentStatusMeta` | `test:client-portal-orders-canonical-data`, `test:client-portal-cp069-fulfillment-display`, `test:prepship-order-lifecycle-parity`, `guard:client-portal-api` | Scope by `clientIds`/`storeIds` (`scope.ts`); financial fields only when `canViewFinancials`; carrier/service identity hard-nulled; carrier telemetry is never a status input (CP-069) |
...
| **Shipments / tracking** | outbound `shipments` only (`outboundShipmentPredicate()`: `is_return = false`, `source <> 'replacement'`) LEFT-JOINED to the linked `orders` row | `GET /shipments` (`?status` in the 5-value contract; legacy `delivered\|in_transit\|exception\|attempted` alias to `shipped` for one release) → `listPortalShipments()` → `toPortalShipmentDto()` (`dto.ts`); `shipmentStatus` = `portalShipmentStatusSql()` (`src/lib/client-portal/shipment-status.ts`): voided → `voided`, else the linked order's effective bucket (`cancelled` / `shipped` / pending → `label_created`), no linkable order → `unavailable`; the same expression projects and filters | `PortalShipment` (`displayTrackingNumber`, `trackingUrl`, `shipDate`, `shipmentStatus` = `shipped \| label_created \| cancelled \| voided \| unavailable`, items, `customerShippingRate`); NO `deliveredAt` / `shipmentStatusDetail` (CP-069) | `pages/Shipments.tsx` (no page-load tracking refresh), `InvoiceShipmentDrawer` | `guard:client-portal-api`, `test:label-shipment-scope-review`, `test:client-portal-cp069-fulfillment-display`, `test:prepship-order-lifecycle-parity` | No raw label URLs/payloads in DTO; scope by `clientIds`/`storeIds`; no carrier telemetry on any outbound surface (CP-069); return labels live on the Returns surface |
| **Inventory quantity / status** | immutable signed `inventory_ledger.qty` sum + `inventory` identity/threshold metadata | `GET /inventory`, `GET /inventory-history` → `listPortalInventory()` → `toPortalInventoryDto()` | `PortalInventory.inventoryQuantity`, backend `stockStatus`, `InventoryMovement` | `pages/Inventory.tsx` renders DTO fields verbatim | `test:ps-439-inventory-sot`, `test:inventory-client-scope`, `guard:client-portal-api` | Scope by `clientIds`/`storeIds`; no stock cache, order-derived fallback, or frontend threshold math |
