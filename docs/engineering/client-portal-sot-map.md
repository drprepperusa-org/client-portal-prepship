...
| **SKU / items / quantity** | `order_items` for reporting; `orders.items` JSON for order display; `inventory` master | `getClientPortalSalesMetrics()` / `getSkuBreakdownFromOrderItems()` for Dashboard and Analysis; `safeItems()` + `isDiscountLine()` for order DTOs | `PortalOrder.items[{sku,name,quantity,imageUrl}]`; `DashboardSummary.units`, `daily[].orderedUnits`, `bySku[]` | `pages/Orders.tsx`, `pages/Dashboard.tsx`, `pages/Analysis.tsx` | `test:client-portal-analytics-parity`, `test:dashboard-bar-chart-top-skus` | `unitPrice` withheld unless `canViewFinancials`; reporting units are complete set-based aggregates |
