...
| Return label PDF | `pdfUrl` | resolved customer-safe URL | `shipments.labelUrl`; `return-label-pdf.ts` validates external object ownership and mints a short-lived private signed URL | label time / response time | backend-owned access projection |
...
| Connection freshness API (retained; no Topbar sync panel) | `connectionStatus`/`lastSyncAt` | GET `/sync-status` | tenant-scoped store connection DTOs; aggregate precedence attention > active > pending > inactive > not connected; timestamp = max successful sync clock | latest account/sync state | derived-from-canonical (backend-owned) |
...
### Saved filter views — Orders, Inventory Stock Levels, Shipments and Returns
...
The existing read models still own membership, status, stock and totals;
...
values are saved. Dates are excluded because these lists do not consume them.
Shipments and Returns use the effective page-local/global client filter for the
storage bucket. Opening a view preserves the Returns order URL constraint.
