...
const fileBytes = '\uFEFF"Inventory ID","SKU"\r\n"1","CSV-1"\r\n"505","CSV-505"\r\n';
async function inventoryFixture(page) {
  const state = { requests: [], exports: [], failure: 0, gate: null };
  await setup(page);
  await page.route('**/api/client-portal/inventory?*',async route=>{
    ...
    if(params.format==='csv') {
      ...
      await route.fulfill({contentType:'text/csv; charset=utf-8',headers:{'Content-Disposition':'attachment; filename="inventory-fixture.csv"'},body:fileBytes});return;
    }
    ...
    await route.fulfill({json:{data:total?[{id:p,sku:`CSV-${p}`,name:'Fixture SKU',clientName:'Alpha',
      inventoryQuantity:7,reorderLevel:10,stockStatus:'low',warehouseShipped30d:3,
      length:null,width:null,height:null,cuFt:null,packageLength:null,active:true}]:[],
...
test('Inventory CSV downloads backend bytes with current client, low-stock, search and sorting across all pages',async({page})=>{
  const state=await inventoryFixture(page);await page.goto(base+'/inventory');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  const first=await download(page);
  expect(first.suggestedFilename()).toBe('inventory-fixture.csv');
  ...
  await page.getByRole('textbox',{name:'Search inventory',exact:true}).fill('CSV');
  ...
  for(const key of ['clientId','lowStock','search','sortBy','sortDir'])expect(exported[key]).toBe(listed[key]);
  expect(exported).toMatchObject({clientId:'1',lowStock:'1',search:'CSV',sortBy:'sku',sortDir:'asc'});
  ...
  await expect(page.getByText('CSV downloaded with all matching inventory. Stock reflects the time of export.')).toBeVisible();
  await page.setViewportSize({width:390,height:844});
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeVisible();
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1)).toBe(true);
  await page.screenshot({path:test.info().outputPath('inventory-export-mobile.png')});
});

test('Inventory export reports progress, failure and limits, permits retry, and does not export an empty table',async({page})=>{
  const state=await inventoryFixture(page);await page.goto(base+'/inventory');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  ...
  await expect(page.getByRole('alert').filter({hasText:'Could not export inventory'})).toBeVisible();
  ...
  await page.getByRole('textbox',{name:'Search inventory',exact:true}).fill('empty');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeDisabled();
});

test('Changing client cancels a pending Inventory export and never downloads the previous client file',async({page})=>{
  const state=await inventoryFixture(page);const downloads=[];page.on('download',file=>downloads.push(file));
  await page.goto(base+'/inventory');await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  ...
  release();state.gate=null;
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  await expect(page.getByText('CSV downloaded with all matching inventory. Stock reflects the time of export.')).toHaveCount(0);
