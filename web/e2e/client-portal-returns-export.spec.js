...
async function returnsFixture(page) {
  const state = { requests: [], exports: [], failure: 0, gate: null };
  await setup(page);
  await page.route('**/api/client-portal/returns?*',async route=>{
    ...
    if(params.format==='csv') {
      ...
      await route.fulfill({contentType:'text/csv; charset=utf-8',headers:{'Content-Disposition':'attachment; filename="returns-fixture.csv"'},body:fileBytes});return;
    }
    ...
    await route.fulfill({json:{data:total?[{id:p,orderNumber:`CSV-${p}`,orderDate:'2026-09-01',clientName:'Alpha',
      returnReference:`CSV-${p}-RETURN`,status:'requested',trackingNumber:null,createdAt:'2026-09-01',returnedSkus:['SKU-A'],returnedQuantity:2}]:[],
...
test('Returns CSV downloads backend bytes with current client, status, search, dates and sorting across all pages',async({page})=>{
  const state=await returnsFixture(page);await page.goto(base+'/returns?order=123');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  const first=await download(page);
  expect(first.suggestedFilename()).toBe('returns-fixture.csv');
  ...
  await page.getByRole('textbox',{name:'Search returns',exact:true}).fill('CSV');
  ...
  for(const key of ['orderId','clientId','status','search','dateFrom','dateTo','sortBy','sortDir'])expect(exported[key]).toBe(listed[key]);
  expect(exported).toMatchObject({orderId:'123',clientId:'2',status:'requested',search:'CSV',sortBy:'order',sortDir:'asc'});
  ...
  await expect(page.getByText('CSV downloaded with all matching returns. Dates are in UTC.')).toBeVisible();
  ...
  await page.screenshot({path:test.info().outputPath('returns-export-mobile.png')});
});

test('Returns export reports progress, failure and limits, permits retry, and does not export an empty table',async({page})=>{
  const state=await returnsFixture(page);await page.goto(base+'/returns');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  ...
  await expect(page.getByRole('alert').filter({hasText:'Could not export returns'})).toBeVisible();
  ...
  await page.getByRole('textbox',{name:'Search returns',exact:true}).fill('empty');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeDisabled();
});

test('Changing client cancels a pending Returns export and never downloads the previous client file',async({page})=>{
  const state=await returnsFixture(page);const downloads=[];page.on('download',file=>downloads.push(file));
  await page.goto(base+'/returns');await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  ...
  release();state.gate=null;
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  await expect(page.getByText('CSV downloaded with all matching returns. Dates are in UTC.')).toHaveCount(0);
