...
const fileBytes = '\uFEFF"Order ID","Order #"\r\n"1","CSV-1"\r\n"505","CSV-505"\r\n';
async function ordersFixture(page) {
  const state = { requests: [], exports: [], failure: 0, gate: null };
  await setup(page);
  await page.route('**/api/client-portal/orders?*',async route=>{
    ...
    if(params.format==='csv') {
      ...
      await route.fulfill({contentType:'text/csv; charset=utf-8',headers:{'Content-Disposition':'attachment; filename="orders-fixture.csv"'},body:fileBytes});return;
    }
    ...
    await route.fulfill({json:{data:total?[{id:p,orderNumber:`CSV-${p}`,orderDate:'2026-09-01',clientName:'Alpha',
      fulfillmentStatus:'pending',orderedUnits:1,items:[],orderTotal:10}]:[],
...
test('Orders CSV downloads backend bytes with current client, status, search, dates and sorting across all pages',async({page})=>{
  const state=await ordersFixture(page);await page.goto(base+'/orders');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  const first=await download(page);
  expect(first.suggestedFilename()).toBe('orders-fixture.csv');
  ...
  await page.getByRole('textbox',{name:'Search orders',exact:true}).fill('CSV');
  ...
  expect(exported).toMatchObject({clientId:'1',status:'shipped',search:'CSV',sortBy:'order',sortDir:'asc'});
  ...
  await expect(page.getByText('CSV downloaded with all matching orders. Dates are in UTC.')).toBeVisible();
  ...
  await page.screenshot({path:test.info().outputPath('orders-export-mobile.png')});
});

test('Orders export reports progress, failure and limits, permits retry, and does not export an empty table',async({page})=>{
  const state=await ordersFixture(page);await page.goto(base+'/orders');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  ...
  await expect(page.getByRole('alert').filter({hasText:'Could not export orders'})).toBeVisible();
  ...
  await page.getByRole('textbox',{name:'Search orders',exact:true}).fill('empty');
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeDisabled();
});

test('Changing client cancels a pending Orders export and never downloads the previous client file',async({page})=>{
  const state=await ordersFixture(page);const downloads=[];page.on('download',file=>downloads.push(file));
  await page.goto(base+'/orders');await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  ...
  release();state.gate=null;
  await expect(page.getByRole('button',{name:'Export CSV',exact:true})).toBeEnabled();
  await expect(page.getByText('CSV downloaded with all matching orders. Dates are in UTC.')).toHaveCount(0);
