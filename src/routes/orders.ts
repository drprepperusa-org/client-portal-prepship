...
} from '../services/order-canonical';
import { computeFulfillmentShiftWindow, formatPtLabel } from '../lib/fulfillment-window';
import { dailyOrderStatusCounts, dashboardSalesRead, type DashboardSalesPayload } from '../services/order-daily-analytics';
import {
  loadOrderListEnrichment,
  mapOrderListRow,
  redactOrderFinancials,
  redactRateMoneyFields,
  type LatestShipmentRow,
} from '../services/orders-list';
import { buildOrderDetailPayload } from '../services/order-detail';
import { buildOrdersExportCsv } from '../services/orders-export';
...
app.get('/daily-counts', zValidator('query', dailyCountsQuery), async (c) => {
  ...
  const rows = await dailyOrderStatusCounts(where);
...
app.get('/dashboard-sales', zValidator('query', dashboardSalesQuery), async (c) => {
  ...
  const { payload, skuRows, dayRows } = await dashboardSalesRead({ where, sevenFrom });
...
app.get('/', zValidator('query', listQuery), async (c) => {
  ...
  const timed = <T>(name: string, fn: () => Promise<T>) => timedOrdersStep(timings, name, fn);
  const { latestShipByOrderId, latestShipByOrderNumber, walmartDirectDuplicateByOrderNumber } =
    await loadOrderListEnrichment({
      pageOrderIds,
      pageOrderNumbers: pageOrderNumbers.filter((n): n is string => Boolean(n)),
      walmartShipStationPageOrderNumbers,
      includeVoided: q.status === 'cancelled',
      timed,
    });
  ...
  const rows = joined
    .map((r) =>
      mapOrderListRow(r, {
        ...
        canViewFinancials,
        latestShipByOrderId,
        latestShipByOrderNumber,
        walmartDirectDuplicateByOrderNumber,
      }),
    )
...
app.get('/export', zValidator('query', exportQuery), async (c) => {
  ...
  const body = buildOrdersExportCsv(q, rows, shipmentsByOrder, shipmentsByOrderNumber, canViewFinancials);
