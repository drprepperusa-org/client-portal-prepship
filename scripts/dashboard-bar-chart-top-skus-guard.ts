// CP-002 + CP-021 guard: Dashboard "Orders over time" is a cumulative
// orders/units BAR chart, and Top SKUs is a DataTable whose numbers come from
// the canonical backend Analysis SKU query, not from folding/sorting a capped
// orders array in the frontend.
...
// Discount/promo lines (negative unit price) are excluded from day units.
{
...
// Top SKUs comes from the canonical backend read-model.
{
  ...
  assert(readModel.includes('getSkuBreakdownFromOrderItems'), 'dashboardTopSkus reuses the canonical Analysis SKU query');
  ...
  assert(/const rows = await db\.select\(\)\.from\(orders\)\.where\(where\)\.limit\(1000\)/.test(route), 'the capped orders array is bounded and used for the visual chart only');
}

// Frontend renders backend Top SKUs verbatim: display cap only, no ranking sort.
{
  const dashboard = read('portal-client/src/pages/Dashboard.tsx');
  assert(
    dashboard.includes('(dash.data?.bySku ?? []).slice(0, 8)') ||
      dashboard.includes('dash.data!.bySku.slice(0, 8)') ||
      dashboard.includes('dash.data.bySku.slice(0, 8)'),
    'Dashboard renders backend-ranked bySku rows in order (display cap only)',
  );
...
// UI: cumulative bar chart, not an area/line chart.
{
  ...
  assert(charts.includes('stackId="ou"') || charts.includes("stackId='ou'"), 'orders/units bars share a stackId');
  assert(charts.includes('unitDelta') && charts.includes('Math.max(0,'), 'continuation segment uses unitDelta = max(0, units - orders)');
...
// UI: Top SKUs is a shared DataTable with the required columns and null dash state.
{
  const dashboard = read('portal-client/src/pages/Dashboard.tsx');
  assert(
    dashboard.includes('<DataTable') &&
      dashboard.includes('tableId="dashboard-top-skus"') &&
      dashboard.includes('allowColumnCustomization={canCustomizeTables}'),
    'Top SKUs renders through the admin-gated shared DataTable',
  );
  ...
  assert(
    dashboard.includes('avgShippingPrice == null') && dashboard.includes('No order carrying this SKU had a shipping charge.'),
    'avg shipping renders a dash state when null instead of $0.00',
  );
  assert(
    dashboard.includes('money(s.avgShippingPrice)') && !dashboard.includes('money(s.shipAlloc)') && !dashboard.includes('s.shipUnits'),
    'Avg Shipping Price cell renders the billed charge without old shipAlloc/shipUnits math',
  );
}

// Honest labels (CP-021): widgets name the entity and table the number is from.
{
  const dashboard = read('portal-client/src/pages/Dashboard.tsx');
  assert(/Shipped orders \(\$\{days\}d\)/.test(dashboard), 'the order-status shipped KPI is labelled "Shipped orders"');
  assert(dashboard.includes('Shipments created'), 'the shipments-table volume chart is labelled "Shipments created"');
...
// Contract + wiring.
{
  ...
  assert(api.includes('billedShippingTotal') && api.includes('chargedUnits'), 'DashboardSummary.bySku exposes billedShippingTotal/chargedUnits');
  ...
  const route = read('src/routes/client-portal/dashboard.ts').replace(/\s+/g, ' ');
  assert(route.includes('dailyOrderUnitsRows') && route.includes('dashboardTopSkus'), 'client-portal route uses per-day aggregation + canonical Top-SKUs read-model');
