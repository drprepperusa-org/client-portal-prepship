...
const INVENTORY_DOWNLOAD = 'portal-client/src/components/inventory/ExportInventoryCsv.tsx';
const ORDER_DOMAIN = 'portal-client/src/lib/api/domains/orders.ts';
const ORDER_DOWNLOAD = 'portal-client/src/components/orders/ExportOrdersCsv.tsx';
...
// Audit, Orders, Inventory and Shipments CSV are generated by the server. Exempt only pinned expressions,
// not whole files: any additional CSV literal in these files must still fail.
...
const ORDER_ACCEPT = `ordersCsv: (token: RequestAuth, opts: ListOpts) => apiBlob(token, '/api/client-portal/orders', {
...
const INVENTORY_ACCEPT = `inventoryCsv: (token: RequestAuth, opts: ListOpts) => apiBlob(token, '/api/client-portal/inventory', {
...
const SHIPMENT_ACCEPT = `shipmentsCsv: (token: RequestAuth, opts: ListOpts) => apiBlob(token, '/api/client-portal/shipments', {
...
for (const file of sourceTreeFiles(['portal-client/src'], root)) {
  ...
  const allowed = allowedBackendMedia.get(rel);
  if (allowed) {
...
ok('media types are restricted to workbook Accept and exact backend audit/Orders/Inventory CSV Accept/type-check expressions');
...
const orderDomain = read(ORDER_DOMAIN).replaceAll('\r\n', '\n');
assert.ok(orderDomain.includes(ORDER_ACCEPT), 'Orders CSV fetcher returns the bare backend apiBlob call');
assert.doesNotMatch(orderDomain, /\.then\(|\bBlob\(|\bFile\(|bytes:|arrayBuffer|\.text\(\)|TextEncoder|btoa\(/,
  'Orders API domain never constructs, chains or reads file bytes');
ok('Orders CSV API returns the backend file without replacing its bytes');

const orderDownload = read(ORDER_DOWNLOAD);
assert.match(orderDownload, /const file = await portalApi\.ordersCsv\(\{ accessToken, signal: controller\.signal \}, filters\);/,
  'Orders download obtains the backend CSV with caller filters and cancellation');
assert.match(orderDownload, /downloadFile\(\{ bytes: file\.bytes, filename: file\.filename \?\? 'orders\.csv' \}\);/,
  'Orders download hands the original Blob to the shared download helper');
assert.equal(count(orderDownload, /downloadFile\(/g), 1, 'Orders download has one file sink');
assert.doesNotMatch(orderDownload, /\bBlob\(|\bFile\(|createObjectURL|TextEncoder|btoa\(|arrayBuffer|\.text\(\)|\.slice\(|\.join\(|JSON\.stringify|file\.bytes\s*=/,
  'Orders UI never assembles, reads, re-encodes or replaces file bytes');
ok('Orders CSV UI preserves backend bytes and cannot become a local builder');

const shipmentDomain = read(SHIPMENT_DOMAIN).replaceAll('\r\n', '\n');
assert.ok(shipmentDomain.includes(SHIPMENT_ACCEPT), 'Shipments CSV fetcher returns the bare backend apiBlob call');
assert.doesNotMatch(shipmentDomain, /\.then\(|\bBlob\(|\bFile\(|bytes:|arrayBuffer|\.text\(\)|TextEncoder|btoa\(/,
  'Shipments API domain never constructs, chains or reads file bytes');
ok('Shipments CSV API returns the backend file without replacing its bytes');

const shipmentDownload = read(SHIPMENT_DOWNLOAD);
assert.match(shipmentDownload, /const file = await portalApi\.shipmentsCsv\(\{ accessToken, signal: controller\.signal \}, filters\);/,
  'Shipments download obtains the backend CSV with caller filters and cancellation');
assert.match(shipmentDownload, /downloadFile\(\{ bytes: file\.bytes, filename: file\.filename \?\? 'shipments\.csv' \}\);/,
  'Shipments download hands the original Blob to the shared download helper');
assert.equal(count(shipmentDownload, /downloadFile\(/g), 1, 'Shipments download has one file sink');
assert.doesNotMatch(shipmentDownload, /\bBlob\(|\bFile\(|createObjectURL|TextEncoder|btoa\(|arrayBuffer|\.text\(\)|\.slice\(|\.join\(|JSON\.stringify|file\.bytes\s*=/,
...
assert.doesNotMatch(inventoryDomain, /\.then\(|\bBlob\(|\bFile\(|bytes:|arrayBuffer|\.text\(\)|TextEncoder|btoa\(/,
  'Inventory API domain never constructs, chains or reads file bytes');
ok('Inventory CSV API returns the backend file without replacing its bytes');

const inventoryDownload = read(INVENTORY_DOWNLOAD);
assert.match(inventoryDownload, /const file = await portalApi\.inventoryCsv\(\{ accessToken, signal: controller\.signal \}, filters\);/,
  'Inventory download obtains the backend CSV with caller filters and cancellation');
assert.match(inventoryDownload, /downloadFile\(\{ bytes: file\.bytes, filename: file\.filename \?\? 'inventory\.csv' \}\);/,
  'Inventory download hands the original Blob to the shared download helper');
assert.equal(count(inventoryDownload, /downloadFile\(/g), 1, 'Inventory download has one file sink');
assert.doesNotMatch(inventoryDownload, /\bBlob\(|\bFile\(|createObjectURL|TextEncoder|btoa\(|arrayBuffer|\.text\(\)|\.slice\(|\.join\(|JSON\.stringify|file\.bytes\s*=/,
  'Inventory UI never assembles, reads, re-encodes or replaces file bytes');
ok('Inventory CSV UI preserves backend bytes and cannot become a local builder');
