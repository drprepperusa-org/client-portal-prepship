...
export function ExportOrdersCsv({ filters, disabled }: { filters: ListOpts; disabled: boolean }) {
  ...
  async function exportCsv() {
    ...
    try {
      const file = await portalApi.ordersCsv({ accessToken, signal: controller.signal }, filters);
      if (controller.signal.aborted) return;
      if (!file.contentType.toLowerCase().startsWith('text/csv')) throw new Error('Unexpected file type');
      downloadFile({ bytes: file.bytes, filename: file.filename ?? 'orders.csv' });
      setMessage('CSV downloaded with all matching orders. Dates are in UTC.');
    } catch (error) {
      if (controller.signal.aborted) return;
