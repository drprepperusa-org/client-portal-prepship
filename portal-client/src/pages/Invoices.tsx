...
export default function Invoices({ from, to }: { from: string; to: string }) {
  ...
  const lineCols: Column<BillingInvoiceDetailRow>[] = useMemo(() => [
    ...
    { key: 'item', header: 'Item Name', defaultWidth: 240, render: (r) => <span className="block truncate text-ink-2" title={r.itemNames ?? ''}>{r.itemNames ?? r.recipientName ?? '—'}</span>, sortAccessor: (r) => r.itemNames ?? '' },
